...
```{r, echo is false, include=TRUE, fig.cap="Barplot of probabilities throwing 0,1 or 2 sixes."}
...
reads <- rpois(n = 3, lambda = 100)
mean(reads)
#df_mle <- data.frame(reads = reads)
...
MLE_function(seq(from = 70, to = 130), reads[1])
```
...
# Dit stukje wil ik graag gebruiken om een opdracht mee te maken dat studenten zelf een gen uit kunnen kiezen om te testen voor differentiatie
```



```{r}
#Maar eerst even een test uitvoeren met gewoon 2 samples die door R zijn gegenereerd. Ik heb hier even 2 means gekozen.
...
# Kun jij me misschien verder helpen Marc? 
```
...
PCA compresses the data by reducing the dimensions in the data. 

```{r, echo=FALSE, include=TRUE, fig.align='center', out.width='50%', fig.cap="Figure x: Reads for 2 correlating samples. A principal component line is fitted through it and the datapoints are projected onto the line"}
knitr::include_graphics(path =here::here("images", "PCA1.png"))
```


```{r, echo=FALSE, include=TRUE, fig.align='center', out.width='50%', fig.cap="Figure x: The line is rotated so the red dots represent the 2 dimensional data (x and y) in 1 dimension (x)"}
...
```{r, echo=FALSE, include=TRUE, fig.align='center', out.width='50%', fig.cap="Figure x: A scree plot showing the eigenvalues of each dimension. Dimension 1 and 2 (PC1 and PC2) are responsible for most variance in the data so a 2 dimensional PCA plot can accurately portray the data."}
