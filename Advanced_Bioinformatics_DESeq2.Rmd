...
Note: This is a project I am currently working on. It is not yet finished
...
All the reads are stored in an expression matrix file (Also referred to as counts data or assay data). This expression matrix is sometimes also included in the expressionset uploaded to GEO but more often than not it is a seperate file included in the supplementary files. Throughout the course you will spend alot of time organizing the data 
...
Before we can run statistical test on the data there are a few steps to go through. These steps are included in the DESeq2 function. This function is basically a "wrapper" function that executes a number of other functions in the "background". The purpose of these functions is roughly the following:
...
In order to adjust for this effect you might propose to scale all reads so the total amount of reads will become equal between samples. This however is not 
...
This result is in line with what we know about the V1 today: It could only be aimed in the general direction of London but not guided to a more specific target. 

## Gamma-Poisson: negative binomial distribution

It turns out the Poisson distrubution alone does not accurately reflect the distribution of genes. The main problem is the lambda in poisson distributions: lambda = mean = variance. 
To accurately model the data we need seperate parameters for mean and variance. 



Estimating dispersion (also called variability or spread) of a gene is hard/impossible if you only have 4 samples per experiment group. RNA seq experiments are quite expensive so just adding samples is not really an option. As a solution DESeq2 assumes genes of similar average expression have a similar dispersion (Love et al 2014).  
...
```{python}
## before you can run this code you need to first install python and then install anaconda. When running this codeblock for the first time Rstudio will ask you if you want to install "reticulate". press yes to continue
...
annot <- getBM(attributes= c('ensembl_gene_id', 'external_gene_name', 'transcript_length'), 
               mart = ensembl)
...
percentagecalculator <- function(bin){
  binres <- filter(res, transcript_length > bin, transcript_length < (bin+50))
  nbinres <- nrow(binres)
  perc_sig <- filter(binres, padj < 0.05)
  npercres <- nrow(perc_sig)
  percentage <- (npercres/nbinres)
return(percentage)
}

allbins <- seq(0, 8900, 100)

percentage_de <- sapply(allbins, percentagecalculator)

length_bias <- data.frame(lengthoftranscript = allbins,
           percentage = percentage_de)

graph3 <- ggplot(data = length_bias, aes(x = lengthoftranscript, y = percentage)) + geom_point() + ylim(0,1)
```
...
res <- filter(res, transcript_length < 10000)

allbins <- seq(0, 152000, 2000)

percentagecalculator <- function(bin){
  binres <- res[bin:(bin+1999),]
  perc_sig <- filter(binres, padj < 0.05)
  npercres <- nrow(perc_sig)
...
lengthcalculator <- function(bin){
  binres <- res[bin:(bin+1999),]
  avg_length <- (sum(binres$transcript_length)/2000)
return(avg_length)
}
...
ggplot(data = length_bias, aes(x = lengthoftranscript, y = percentage)) + geom_point()

graph4 <- ggplot(data = length_bias, aes(x = lengthoftranscript, y = percentage)) + geom_point() + ylim(0,1)

```

```{r}
#grid.arrange(graph1, graph2, graph3, graph4, ncol=4)
```
